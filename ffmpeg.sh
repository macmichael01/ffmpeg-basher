...
export MUX_EXT=(flv avi mkv 3gp ogv m4v f4v mov mpeg mpg ts m2ts vob mp4 wmv)
...
# Lower is better. Default is 20.
# This settings is only for WMV Transcoding, since there doesn't
# Appear to be a way to mux audio and video from a WMV file.
export MUX_CRF=20
...
function muxvideo() {
  ...
  if [[ ! $mux_extension -eq "wmv" ]]; then
    # Transcodes WMV videos files into an mp4 container. Converts audio to AAC lossless.
    ffmpeg -i "${mux_filename}.${mux_extension}" -c:v libx264 -crf $MUX_CRF -c:a libfaac -q:a 100 "${mux_filename}.${MUX_TO}" -nostdin ${mux_report} ${mux_verbose}
  else
    # For all other formats attempt to mux the audio and video containers specified by the user.
